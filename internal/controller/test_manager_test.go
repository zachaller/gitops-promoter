...
func startPartitionedManager(ctx context.Context, cfg *rest.Config, namespace string, instanceID *string) context.CancelFunc {
	// The startup instance ID is process-global state also read by the main suite's managers
	// (started in BeforeSuite against the primary envtest). Swap it for this partitioned
	// manager's lifetime and restore it in stop(), otherwise every reconcile in later specs on
	// this ginkgo process fails ensureControllerInstanceIDStable with a drift error.
	restoreInstanceID := settings.SetStartupInstanceIDForTest(instanceID)
