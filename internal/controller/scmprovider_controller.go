...
func (r *ScmProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	defer func() {
		...
		applyConfig := acv1alpha1.ScmProvider(scmProvider.Name, scmProvider.Namespace).WithStatus(statusApply)
		utils.HandleSSAReconciliationResult(ctx, startTime, &scmProvider, utils.SSAReconciliationParams{
			ApplyConfig:      applyConfig,
			AppendConditions: func(c ...*acmetav1.ConditionApplyConfiguration) { statusApply.WithConditions(c...) },
			Client:           r.Client,
			FieldOwner:       constants.ScmProviderControllerFieldOwner,
			Recorder:         r.Recorder,
			FallbackFactory: func(name, ns string, c ...*acmetav1.ConditionApplyConfiguration) any {
				return acv1alpha1.ScmProvider(name, ns).WithStatus(acv1alpha1.ScmProviderStatus().WithConditions(c...))
			},
		}, &result, &err)
	}()
